...
#include "../App/Tools/ezOptionParser.h"
#include "../App/Exceptions/Exceptions.h"
#include "../App/Networking/ServerSocket.h"
#include "../App/Networking/sockets.h"
#include "../App/Networking/data.h"
#include "../App/Tools/time-func.h"
...
#include "../App/Tools/json.hpp"
#include "../App/Tools/common.h"
#include "../App/CTools/hexutil.h"
#include "../App/CTools/fileio.h"
#include "../App/CTools/crypto.h"
#include "../App/CTools/byteorder.h"
#include "../App/Tools/msgio.h"
#include "../App/Tools/protocol.h"
#include "../App/CTools/base64.h"
#include "../App/Tools/iasrequest.h"
#include "../App/CTools/logfile.h"
#include "../App/Tools/settings.h"
#include "../App/CTools/enclave_verify.h"
...
/*generate msg2, using get_sigrl to get the sigrl used in msg2*/
...
/*generate msg4, using get_attestation_report to get the attestation report*/
...
int get_attestation_report(IAS_Connection *ias, int version,
    ...
    int strict_trust) 
{
    edividerWithText("get into get_attestation_report successfully!");
    IAS_Request *req = NULL;
    ...
    edividerWithText("payload insert");
...
client_core::client_core(int argc, const char** argv){
    ...
    opt.syntax = "./client.x -nt 1 -cn 0 -h localhost -pn 5000\n";
    opt.add(
        "", // Default.
        0, // Required?
        1, // Number of args expected.
        0, // Delimiter if expecting multiple args.
        "Directory containing the client data (default: Client_Storage+client_number)", // Help description.
        "-d", // Flag token.
        "--dir" // Flag token.
    );
    opt.add(
    ...
    opt.add(
        ...
        1, // Number of args expected.
        0, // Delimiter if expecting multiple args.
        "client number (default 0)", // Help description.
        "-cn", // Flag token.
        "--client_number" // Flag token.
    );
    opt.add(
        "localhost", // Default.
        0, // Required?
    ...
    opt.get("--client_number")->getInt(mynumber);
    ...
    if (opt.isSet("--dir"))
    {
        ...
        PREP_DATA_PREFIX = "Client_Storage" + to_string(mynumber) + "/";
    }
    ...
    */
    int port_num = pnbase + mynumber*nthreads*sgx_mpsi::port_increase;
    int inst = -1;
    int thread_sign;
    ...
    for (int i=0; i<nthreads; i++)
    {
        ...
        thread_sign = mynumber*nthreads + i;
        send(socket_num[i], (octet*)&thread_sign, sizeof(thread_sign));
        cout << "Sent " << thread_sign << " to " << hostname.c_str() << ":" << port_num << endl;
    ...
    */
    transfer_buffer.resize(nthreads);
    transfer_ptr.resize(nthreads);
    for(int i=0; i<nthreads; i++){
        transfer_buffer[i] = new uint8_t[sgx_mpsi::max_TransferBytes];
        transfer_ptr[i] = 0;
    }
...
//      {   
//         //cout<<"src["<<i<<"]:"<<decToBin((size_t)src[i])<<endl;   
//         final.append(std::to_string(decToBin((size_t)src[i])));
//      }
//     data=binToDec(final);
//     //cout<<sizeof(uint8_t)<<endl;
//     //copy(&src[0], &src[7] , &data);    //-2 ???? 栈溢出
//    //copy(src.begin(), src.end(), &data);
//     }
// void unpack(vector <uint8_t >& src, size_t index, size_t& data){
//     copy(&src[index], &src[index -1 + sizeof (size_t)], &data);    //-2 ???? 栈溢出
//     }
...
// 公钥加密    
string rsa_pub_encrypt(const string &clearText,string &pubKey)  
{  
    std::string strRet;  
...
void client_core::init()
{
    cout<<"client is doing init now including (receiveing data from server)"<<endl; 
    //cout<<"+++:"<<att<<endl;

    size_t len_i=0;

    uint8_t *inst_1=new uint8_t[20000];
    size_t len_1;
    receive_from(0, inst_1,len_1); 
    unpack(inst_1, len_1, len_i);
    //cout<<"len_i:"<<len_i<<endl;

    size_t *x=new size_t[len_i];
    for(int k=0;k<len_i;k++)
        x[k]=0;

    for(int n=0;n<len_i;n++)
     {
        uint8_t *inst=new uint8_t[200];
        size_t len;
        receive_from(0, inst,len); 
        unpack(inst,len,x[n]);
        //cout<<"X_receive["<<n<<"]:"<<x[n]<<endl;
     }
...
void client_core::start(size_t *X,size_t len){
    ...
    // cout<<"Client "<<mynumber<<" start now\n";  

    // send(socket_num[0],ATT);
   ...
   //      pack(buff_2,X[n]);
   //      uint8_t *test_2=new uint8_t[buff_2.size()];
   //      for(int i=0;i<buff_2.size();i++)  
   //          test_2[i]=buff_2[i];
   //      send_to(0, test_2 ,buff_2.size());
   // }

    cout<<"client"<<endl;

    /*getpk()*/
    send(socket_num[0], getpk); 
    cout<<"1-client send the signal 'getpk' "<<endl;
    uint8_t *inst=new uint8_t[2000];
    size_t l;
    receive_from(0, inst,l); 
    //cout<<l<<endl;
    cout<<"1-client receive the publickey:"<<endl;
    string publickey(inst, inst+l);
    cout<<publickey;
    ...
    while (sig2 != exKey) 
    { 
        receive(socket_num[0], sig2); 
    }
    cout<<"2-client receive the signal 'exKey' "<<endl;
    string key="123";
    string lyj=rsa_pub_encrypt(key,publickey);
    //cout<<"123lyj:"<<lyj<<endl;
    send_to(0,(uint8_t *)lyj.data(),lyj.size());
    ...
    while (sig3 != setsize) 
    { 
        receive(socket_num[0], sig3); 
    }
    cout<<"3-client receive the signal 'setsize' "<<endl;
    std::ostringstream ss;
    ss << len;
    string messagelen=des_encrypt(ss.str(),key);
    messagelen.append("+");
    messagelen.append(ss.str());
    send_to(0,(uint8_t *)messagelen.data(),messagelen.size());
    // char length_m[200];
    // char length_c[200];
    // sprintf(length_m, "%d", len);
    // int aes=aes256_encrypt(length_m,length_c);
    // cout<<aes<<endl;
    // cout<<length_c<<endl;
    // send_to(0,(uint8_t *)length_c,strlen(length_c));
