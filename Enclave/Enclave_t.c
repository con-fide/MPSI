...
static sgx_status_t SGX_CDECL sgx_msi(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_msi_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_msi_t* ms = SGX_CAST(ms_msi_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	size_t*** _tmp_X = ms->ms_X;
	size_t _tmp_n = ms->ms_n;
	size_t _len_X = _tmp_n * sizeof(size_t**);
	size_t*** _in_X = NULL;
	size_t** _tmp_len = ms->ms_len;
	size_t _len_len = _tmp_n * sizeof(size_t*);
	size_t** _in_len = NULL;
	size_t* _tmp_out = ms->ms_out;
	size_t _tmp_len_min = ms->ms_len_min;
	size_t _len_out = _tmp_len_min * sizeof(size_t);
	size_t* _in_out = NULL;
	...
	if (sizeof(*_tmp_len) != 0 &&
		(size_t)_tmp_n > (SIZE_MAX / sizeof(*_tmp_len))) {
		return SGX_ERROR_INVALID_PARAMETER;
	}

	if (sizeof(*_tmp_out) != 0 &&
		(size_t)_tmp_len_min > (SIZE_MAX / sizeof(*_tmp_out))) {
		return SGX_ERROR_INVALID_PARAMETER;
	}
	...
	if (_tmp_len != NULL && _len_len != 0) {
		...
		_in_len = (size_t**)malloc(_len_len);
		if (_in_len == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_len, _len_len, _tmp_len, _len_len)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	msi(_in_X, _in_len, _in_out, _tmp_n, _tmp_len_min, ms->ms_n_bucket);
	if (_in_out) {
